...
export default {
  ...
  methods: {
    ...
    pushInVariant(product) {
      this.pvariants()
      product.product_variant_prices.forEach(item => {
        if (item.product_variant_one != null) {
          this.product_variant[0].tags.push([
            item.product_variant_one.variant_name
          ])
        }
        if (item.product_variant_two != null) {
          this.product_variant[1].tags.push([
            item.product_variant_two.variant_name
          ])
        }
        if (item.product_variant_three != null) {
          this.product_variant[2].tags.push([
                item.product_variant_three.variant_name
              ]
          )
        }
      })
  ...
  mounted() {
    ...
    this.pushInVariant(this.product)
  }
